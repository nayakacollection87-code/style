...
export default function ShopeeServicePage() {
  ...
  const userOrders = user ? getUserTrafikOrders(user.id).filter(o => o.platform === "shopee") : [];
  const isPending = user ? hasPendingOrder(user.id, "shopee") : false;
  ...
  const handleSubmit = async () => {
    ...
    if (!validateUrl(url, "shopee")) {
      setError("URL tidak valid. Masukkan URL video Shopee yang benar");
      return;
    }
    ...
    addKoinHistory({
      ...
      type: "usage",
      amount: -koinNeeded,
      description: `Order Shopee ${viewCount} views`,
      createdAt: new Date(),
    });
      ...
      >
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-r from-orange-500/20 to-orange-500/10 flex items-center justify-center">
              <ShoppingBag className="w-5 h-5 text-orange-500" />
            </div>
            <h1 className="text-2xl lg:text-3xl font-bold text-foreground">Shopee Service</h1>
          </div>
          <p className="text-muted-foreground">
            Tingkatkan view video Shopee Anda dengan layanan trafik kami
          </p>
        </div>
